...
$$
\begin{split}
\mathbb{P}\left(5000 < L \right) = 1-\mathbb{P}\left(K_{[x]+1}>\underbrace{\frac{\ln\left(\frac{S+\frac{P}{d}}{5000+\frac{P}{d}}\right)}{\ln(1+i)}}_{\tau} \right)
\end{split}
$$
Hay que tener presente que pueden obtener números no enteros, por lo que se debe tomar $\floor{\lfloor\tau\rfloor}$, entonces lo anterior es $1-_\floor{\lfloor\tau\rfloor}p_{x}$. Entonces se puede programar la siguiente función, notanto que esto es la probabilidad de que el individuo sobreviva esa cantidad de tiempo
```{r}
...
```
Entonces las probabilidades de tener pérdidades menores a `r p_0[1]` y la edad es `r 50+p_0[2]`.