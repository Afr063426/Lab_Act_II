...
distribution%>%ggplot(aes(x=Valor_de_las_simulaciones,fill=Seguro))+geom_histogram(bins=100)+facet_wrap(~Seguro,scales='free')+scale_fill_jcolors(palette = "pal2") +
    theme_minimal()
```
...
```{r}
#Se prococede a estimar cada uno de los valores para los seguros de acuerdo a cada temporalidad
a30<-numeric()
a40<-numeric()
a30_40<-numeric()
a_30_40<-numeric()
...
annuities%>%ggplot(aes(x=Temporalidad,y=Valor,color=Anualidad))+geom_line()+scale_fill_jcolors(palette = "pal2") +
    theme_minimal()
```
...
distribution_axy%>%ggplot(aes(x=Valor_de_las_simulaciones,fill=Anualidad))+geom_histogram(bins=100)+facet_wrap(~Anualidad,scales='free')+scale_fill_jcolors(palette = "pal2") +
    theme_minimal()
```
...
#Anualidad especial
#Deterministico
a30_40_sp<-1/3*a30_40+2/3*a_30_40
```
...
axy_sp%>%ggplot(aes(x=a30_40_sp_s,fill=''))+geom_histogram(bins=100)+scale_fill_jcolors(palette = "pal2") +
    theme_minimal()
...
## Deterministico
```{r}
p30_40<-A30_40/axn(life_table,x=30,i=interest)

##Ultimo sobreviviente
p_30_40<-A_30_40_s/axn(life_table,x=30,i=interest)

```
El resultado obtenido es 
$$
Prima_{30:40}=`r p30_40` \qquad Prima_{\overline{30:40}}=`r p_30_40`
$$
## Estocástico
```{r}
#Vida individuales
...
El resultado obtenido es 
$$
Prima_{30:40}=`r sum(p30_40_s)/simulations` \qquad Prima_{\overline{30:40}}=`r sum(p_30_40_s)/simulations`
$$
...
distribution_pxy%>%ggplot(aes(x=Valor_de_las_simulaciones,fill=Prima))+geom_histogram(bins=100)+facet_wrap(~Prima,scales='free')+scale_fill_jcolors(palette = "pal2") +
    theme_minimal()
```
...
vT30_s<-vT30_s[vT30_s<=quan]
p30<-mean(vT30_s)/mean(a30_v)
quan<-quantile(vT40_s,0.45)
vT40_s<-vT40_s[vT40_s<=quan]
p40<-mean(vT40_s)/mean(a40_v)

#Vida conjunta
quan<-quantile(vT30_40_s,0.45)
vT30_40_s<-vT30_40_s[vT30_40_s<=quan]
p30_40<-mean(vT30_40_s)/a30_40_s
#Ultimo sobreviviente
quan_last<-quantile(vT_30_40_s,0.45)
vT_30_40_s<-vT_30_40_s[vT_30_40_s<=quan]
p_30_40<-mean(vT_30_40_s)/a_30_40_s
...
```{r}
#Se contruyen las probabilidades con la funcion 
