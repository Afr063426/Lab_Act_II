...
reserve_t_1 <- benefit_1 * Axn(life_table, x = 40 + t, i = 0.105, n = 9 + rev(t)) + benefit_2 * v^(9 + rev(t)) * pxt(life_table, x = 40 + t, t = 9 + rev(t)) - risk_p * axn(life_table, n = rev(t), x = 40 + t, i = 0.105, payment = "advance")
##
...
status %>% ggplot(aes(x = Tiempo, y = Probabilidad, color = Status)) +
    geom_line(size = 1) +
    scale_color_jcolors(palette = "pal2", labels = unname(TeX(c("$_tp_{30}$", "$_tp_{40}$", "$_tp_{30/40}$", "$_tp_{\\bar{30/40}}$")))) +
    theme_minimal()
```
...
$$
A_{\overline{xy}}=\sum_{k=0}^{\infty}v^{k+1}(_kp_{x}q_{x+k}+_kp_{y}q_{x+k}-_kp_{xy}q_{x+k:y+k})\qquad A_{x:y}=A_{x}+A_{y}-A_{\overline{xy}}
$$
...
v_2 <- 1 / (1 + i)
# Seguro vida 30 años
Ax <- sum(v_2^k * unlist(lapply(FUN = tpx, x = x, X = k)) * (1 - unlist(lapply(FUN = tpx, X = k + x, t = 1))))

Ay <- sum(v_2^k * unlist(lapply(FUN = tpx, x = y, X = k)) * (1 - unlist(lapply(FUN = tpx, X = k + y, t = 1))))
```
