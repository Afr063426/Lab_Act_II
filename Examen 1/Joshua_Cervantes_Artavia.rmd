...
L1 <- benefit_1 * v_acum[1:10] - amount_payments[1:10]
L2 <- benefit_1 * v_acum[11:20] - amount_payments[10]
...
E_premium <- c((E_survival[1:10] * risk_p), rep(0, 10))
E_reclaims <- -diff(lx40_60) * benefit_1
E_i <- E_premium[1] * 0.105
...
t <- 1:10
# Sumar 9
# benefit_1 * Axn(life_table, x = 42, i = i, n = 18) + benefit_2 * v^(18) * pxt(life_table, x = 42, t = 18) - risk_p * axn(life_table, n = 8, x = 42, i = i, payment = "advance")
reserve_t_1 <- benefit_1 * Axn(life_table, x = 40 + t, i = 0.105, n = 9 + rev(t)) + benefit_2 * v^(9 + rev(t)) * pxt(life_table, x = 40 + t, t = 9 + rev(t)) - risk_p * axn(life_table, n = rev(t), x = 40 + t, i = interest, payment = "advance")
##
...
$$
Y se recuerda que si se suponen las vidas independientes
$$
_tp_{xy}=_tp_{x}_tp_{y}\qquad _tp_{\overline{xy}}=_tp_{x}+_tp_{y}-_tp_{x}_tp_{y}
$$
...
<caption><center><strong>Figura 2: Probabilidades de sobrevivencia </strong></center></caption>
```{r}
...
status %>% ggplot(aes(x = Tiempo, y = Probabilidad, color = Status)) +
    ...
    theme_minimal()
# labels = unname(TeX(c("$_tp_{30}$", "$_tp_{40}$", "$_tp_{30/40}$", "$_tp_{\\bar{30/40}}$")))
```
...
Entonces se cumple lo siguiente con lo desarrollado por nosotros
$$
A_{30}=`r Ax`\qquad A_{40}=`r Ay` \qquad A_{30:40}=`r Axy`\qquad A_{\overline{xy}}=`r A_xy`
$$
...
$$
\"a_{xy}=\sum_{k=0}^{\infty}v^k_kp_{xy}\qquad \"a_{\overline{xy}} =\"a_x+\"a_y-\"a_{xy}
$$
